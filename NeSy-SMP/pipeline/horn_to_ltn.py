...
def _load_relation_semantics(relation_vocab_path: str | Path | None):
    """관계별 컴파일 방식과 결과 개념 목록. vocab 이 없으면 기존(sepsis) 동작."""
    default_sem = {
        "increasesRiskOf": {"kind": "implication", "negate_head": False},
        "increaseRiskOf": {"kind": "implication", "negate_head": False},
    }
    default_out = ("Death", "HighMortality", "Outcome")
    if relation_vocab_path is None:
        p = Path(__file__).resolve().parents[1] / "configs" / "relation_vocab.json"
        relation_vocab_path = p if p.exists() else None
    if relation_vocab_path is None:
        return default_sem, default_out
    with open(relation_vocab_path, encoding="utf-8") as f:
        vocab = json.load(f)
    sem = vocab.get("relation_semantics") or default_sem
    out = tuple(vocab.get("outcome_concepts") or default_out)
    return sem, out


def compile_implications_from_kg(
    concepts_path: str | Path,
    relation_vocab_path: str | Path | None = None,
) -> List[AxiomIR]:
    """
    Direct KG path (no AnyBURL):
      (RiskFactor, increasesRiskOf, Death)      -> forall x Pred(x) -> HighMortality(x)
      (Treatment,  decreasesRiskOf, Delirium)   -> forall x Pred(x) -> NOT Delirium(x)
      (DeepSedation, precludes, Assessable)     -> forall x Pred(x) -> NOT Assessable(x)
      (X, hasNoEffectOn, Delirium)              -> 공리 아님. 마이닝 규칙 기각 필터.

    PADIS 는 예방 권고가 절반이라 음의 방향 관계가 없으면 술어가 전부 양의 방향으로
    쏠린다 (원 논문 Table 3 의 LTN-AK collapse). 그래서 negate_head 를 분리했다.
    """
    with open(concepts_path, encoding="utf-8") as f:
        cfg = json.load(f)
    sem, outcomes = _load_relation_semantics(relation_vocab_path)
    ...
    for t in cfg["triples"]:
        ...
        spec = sem.get(rel)
        if spec is None:
            # 기존 동작 유지: 이름에 risk 가 들어가면 위험 증가로 본다
            if "risk" not in rel.lower():
                continue
            spec = {"kind": "implication", "negate_head": False}
        if not spec.get("skip_outcome_check") and t["object"] not in outcomes:
            continue
        body = resolve_predicate(t["subject"], concepts_path)
        head = resolve_predicate(t["object"], concepts_path)
        bp, hp_ = body["predicate"], head["predicate"]
        neg = bool(spec.get("negate_head"))
        kind = spec.get("kind", "implication")
        head_txt = f"\u00ac{hp_}(x)" if neg else f"{hp_}(x)"
        formula = (f"REJECT: {bp}(x) -> {hp_}(x)" if kind == "filter"
                   else f"\u2200x: {bp}(x) \u2192 {head_txt}")
        axioms.append(
            AxiomIR(
                kind=kind,
                formula=formula,
                head_predicate=hp_,
                body_predicates=[bp],
                source=f"kg:{t['subject']}-{rel}-{t['object']}",
                code_predicates=[body.get("code_predicate", bp), head.get("code_predicate", hp_)],
                relation=rel,
                negated_head=neg,
            )
        )
