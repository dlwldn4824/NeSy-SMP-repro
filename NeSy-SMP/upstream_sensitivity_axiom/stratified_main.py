import os, sys, json
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))   # [PATCH P1] cwd 를 결과 폴더로 바꿔도 import 되게
import pandas as pd
...
DATA_PATH = os.environ.get("NESY_DATA", "data/subset/events_48h_before_death_gcs.csv")   # [PATCH P1]
data = pd.read_csv(DATA_PATH, dtype={"hadm_id": str, "subject_id": str})
print(data.columns)
...
for i, (train_index, test_index) in enumerate(skf.split(X_all, y_all)):
    ...
    for x, y in zip(X_train, y_train):
        ...
        features_std = [std_excluding_zeros(lst) for lst in features]   # [PATCH P5] 원래 평균값 리스트로 표준편차를 구해 TypeError
    ...
    creatinine_above_threshold = lambda x: (((x[:,(features_dict["Creatinine (serum)"]*sequence_length-sequence_length):(features_dict["Creatinine (serum)"]*sequence_length)] >= scalers["Creatinine (serum)"].transform(np.array([[1.5]]))[0][0]) | ((x[:,(features_dict["Creatinine (serum)"]*sequence_length-sequence_length):(features_dict["Creatinine (serum)"]*sequence_length)] == 0) | (x[:,(features_dict["Creatinine (serum)"]*sequence_length-sequence_length):(features_dict["Creatinine (serum)"]*sequence_length)] == scalers["Creatinine (serum)"].transform(np.array([[0.0]]))[0][0]))).all(axis=1) & ((x[:,(features_dict["Creatinine (serum)"]*sequence_length-sequence_length):(features_dict["Creatinine (serum)"]*sequence_length)] != 0) & (x[:,(features_dict["Creatinine (serum)"]*sequence_length-sequence_length):(features_dict["Creatinine (serum)"]*sequence_length)] != scalers["Creatinine (serum)"].transform(np.array([[0.0]]))[0][0])).any(axis=1))   # [PATCH S1]
    respiratory_rate_risk = lambda x: ((((x[:,(features_dict["Respiratory Rate"]*sequence_length-sequence_length):(features_dict["Respiratory Rate"]*sequence_length)] >= scalers["Respiratory Rate"].transform(np.array([[29.0]]))[0][0]) | ((x[:,(features_dict["Respiratory Rate"]*sequence_length-sequence_length):(features_dict["Respiratory Rate"]*sequence_length)] == 0) | (x[:,(features_dict["Respiratory Rate"]*sequence_length-sequence_length):(features_dict["Respiratory Rate"]*sequence_length)] == scalers["Respiratory Rate"].transform(np.array([[0.0]]))[0][0]))).all(axis=1) | (((x[:,(features_dict["Respiratory Rate"]*sequence_length-sequence_length):(features_dict["Respiratory Rate"]*sequence_length)] < scalers["Respiratory Rate"].transform(np.array([[9.0]]))[0][0]) & ((x[:,(features_dict["Respiratory Rate"]*sequence_length-sequence_length):(features_dict["Respiratory Rate"]*sequence_length)] != 0) & (x[:,(features_dict["Respiratory Rate"]*sequence_length-sequence_length):(features_dict["Respiratory Rate"]*sequence_length)] != scalers["Respiratory Rate"].transform(np.array([[0.0]]))[0][0]))) | ((x[:,(features_dict["Respiratory Rate"]*sequence_length-sequence_length):(features_dict["Respiratory Rate"]*sequence_length)] == 0) | (x[:,(features_dict["Respiratory Rate"]*sequence_length-sequence_length):(features_dict["Respiratory Rate"]*sequence_length)] == scalers["Respiratory Rate"].transform(np.array([[0.0]]))[0][0]))).all(axis=1)) & ((x[:,(features_dict["Respiratory Rate"]*sequence_length-sequence_length):(features_dict["Respiratory Rate"]*sequence_length)] != 0) & (x[:,(features_dict["Respiratory Rate"]*sequence_length-sequence_length):(features_dict["Respiratory Rate"]*sequence_length)] != scalers["Respiratory Rate"].transform(np.array([[0.0]]))[0][0])).any(axis=1))   # [PATCH S1]
    blood_pressure_systolic_risk = lambda x: ((x[:,(features_dict["Arterial Blood Pressure systolic"]*sequence_length-sequence_length):(features_dict["Arterial Blood Pressure systolic"]*sequence_length)] <= scalers["Arterial Blood Pressure systolic"].transform(np.array([[100.0]]))[0][0]) & ((x[:,(features_dict["Arterial Blood Pressure systolic"]*sequence_length-sequence_length):(features_dict["Arterial Blood Pressure systolic"]*sequence_length)] != 0) & (x[:,(features_dict["Arterial Blood Pressure systolic"]*sequence_length-sequence_length):(features_dict["Arterial Blood Pressure systolic"]*sequence_length)] != scalers["Arterial Blood Pressure systolic"].transform(np.array([[0.0]]))[0][0]))).any(axis=1)   # [PATCH S1]
    blood_pressure_systolic_risk = lambda x: ((x[:,(features_dict["Arterial Blood Pressure systolic"]*sequence_length-sequence_length):(features_dict["Arterial Blood Pressure systolic"]*sequence_length)] <= scalers["Arterial Blood Pressure systolic"].transform(np.array([[100.0]]))[0][0]) & ((x[:,(features_dict["Arterial Blood Pressure systolic"]*sequence_length-sequence_length):(features_dict["Arterial Blood Pressure systolic"]*sequence_length)] != 0) & (x[:,(features_dict["Arterial Blood Pressure systolic"]*sequence_length-sequence_length):(features_dict["Arterial Blood Pressure systolic"]*sequence_length)] != scalers["Arterial Blood Pressure systolic"].transform(np.array([[0.0]]))[0][0]))).any(axis=1)   # [PATCH S1]
    ...
    low_gcs = lambda x: ((x[:,(features_dict["gcs"]*sequence_length-sequence_length):(features_dict["gcs"]*sequence_length)] < scalers["gcs"].transform(np.array([[8.0]]))[0][0]) & ((x[:,(features_dict["gcs"]*sequence_length-sequence_length):(features_dict["gcs"]*sequence_length)] != 0) & (x[:,(features_dict["gcs"]*sequence_length-sequence_length):(features_dict["gcs"]*sequence_length)] != scalers["gcs"].transform(np.array([[0.0]]))[0][0]))).any(axis=1)   # [PATCH S1] 0 은 측정 없음
    platelet_below_threshold = lambda x: (((x[:,(features_dict["Platelet Count"]*sequence_length-sequence_length):(features_dict["Platelet Count"]*sequence_length)] < scalers["Platelet Count"].transform(np.array([[50.0]]))[0][0]) | ((x[:,(features_dict["Platelet Count"]*sequence_length-sequence_length):(features_dict["Platelet Count"]*sequence_length)] == 0) | (x[:,(features_dict["Platelet Count"]*sequence_length-sequence_length):(features_dict["Platelet Count"]*sequence_length)] == scalers["Platelet Count"].transform(np.array([[0.0]]))[0][0]))).all(axis=1) & ((x[:,(features_dict["Platelet Count"]*sequence_length-sequence_length):(features_dict["Platelet Count"]*sequence_length)] != 0) & (x[:,(features_dict["Platelet Count"]*sequence_length-sequence_length):(features_dict["Platelet Count"]*sequence_length)] != scalers["Platelet Count"].transform(np.array([[0.0]]))[0][0])).any(axis=1))   # [PATCH S1]
...
with open("fold_metrics.json", "w") as _f:   # [PATCH P2]
    _g = dict(globals())
    json.dump({k: [float(v) for v in _g[k]] for k in _g
               if k.endswith(("_accuracies", "_f1_scores", "_precisions", "_recalls", "_roc_aucs"))}, _f, indent=1)
