...
> **개념 병목의 비용은 대부분 우리 인코딩 탓이었습니다.**
> 이진 임계를 연속값으로 바꾸니 −17.5 → −8.0 → **−3.2** 로 줄었습니다.
> **논리층이 얹힐 개념 공간은 걱정한 것보다 훨씬 충실합니다.**
...
6. **개념 병목 비용은 −3.2 까지 줄었다** (연속 인코딩). 논리층이 물려받는 손실이 그만큼이다. 남은 −3.1 은 개념 밖 신호의 추정치인데, **5단계를 미룰 만한 크기는 아니다.**
...
**순서 제안**: (a) 개념 정의 정교화까지 끝났다 — 병목 비용이 −17.5 → −3.2 로 줄어 **5단계의 전제가 확보됐다.**
남은 임계경로는 **(b) 컴파일된 공리 19개를 학습 손실에 연결하는 것** 하나다.
연속 개념 세트(17개)를 논리층 인터페이스로 쓰면 되고, `horn_to_ltn` 가 내놓는 술어명과 개념 이름을 맞추는 작업이 먼저 필요하다.
**6단계(LNN)는 4↔5 게이트 통과 후에만.**
