...
> **3단계 BiLSTM 은 45.4 로 XGBoost 를 넘지 못했고, 이번엔 채널이 다 찬 상태라 확정입니다.**
...
| 계층 | 기저% | 1 lookup | **2 XGB** | 3 BiLSTM | 4 CBM | 4b CBM+Assess |
|---|---:|---:|---:|---:|---:|---:|
| 전체 | 31.0 | 71.6 | **82.8** | 81.6 | 49.1 | 51.7 |
| **앵커=N** (주 지표) | 12.0 | 23.0 | **48.9** | 45.4 | 27.9 | 27.2 |
| 앵커=P | 78.1 | 83.0 | **90.6** | 89.8 | 82.9 | 81.0 |
| 앵커=U | 53.3 | 65.5 | **82.3** | 81.0 | 63.1 | 63.2 |

앵커=N AUROC: 63.7 → **85.2** → 83.5 → 66.6 → 64.7
...
| 비교 | Δ | 읽는 법 |
|---|---:|---|
| 2 − 1 | **+25.9** | 요약 피처가 룩업표를 크게 넘는다 |
| 3 − 2 | **−3.5** | **원시 시계열이 요약을 못 넘었다 — 채널이 다 찬 상태의 확정 결과** |
| 4 − 3 | **−17.5** | 개념 병목의 비용. 개념 5개(6개 중, Mobility·Pain 개념은 미정의)라 상한값 |
| 4b − 4 | **−0.7** | 미관찰 앵커를 Assessable 로 살려도 **이득이 없다** |

> 🔴 **3단계 결론이 바뀐다.** 이전 실행(채널 반쪽)에서 −2.7 이던 것이 채널을 다 채우고도 −3.5 다.
> **BiLSTM 은 이 과제에서 XGBoost 를 못 넘는다.** 표 형태 데이터에서 GBDT 가 이기는 흔한 양상이고,
> 여기서는 24h 원시 시퀀스가 요약 통계 이상을 담고 있지 않다는 뜻이다.
> → **4~6단계 backbone 을 BiLSTM 이 아니라 요약 피처 위의 MLP 로 잡는 것을 제안한다.**
> 논리층은 미분 가능한 backbone 이 필요하니 XGBoost 를 그대로 쓸 수는 없지만,
> 같은 피처를 MLP 로 받으면 시퀀스 인코더 없이 XGB 에 훨씬 가깝게 갈 수 있다.
