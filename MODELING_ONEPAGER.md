...
재현 스크립트: `eda/20_` baseline · `eda/21_` 검열원인 · `eda/22_` XGB · `eda/23_` BiLSTM·CBM
...
> **4단계까지 돌렸습니다. 23.0 → 46.1(XGB)** 로 두 배가 됐고, **3·4단계는 그걸 넘지 못했습니다.**
> 다만 mobility/pain/GCS 가 빠진 상태라 시계열·개념층에 불리한 조건입니다.
...
|---|---:|---:|---:|---:|---:|---:|
| 전체 | 31.0 | 71.6 | **82.0** | 80.8 | 72.1 | 69.4 |
| **앵커=N** (주 지표) | 12.0 | 23.0 | **46.1** | 43.4 | 29.5 | 25.6 |
| 앵커=P | 78.1 | 83.0 | **90.0** | 89.3 | 84.6 | 83.3 |
| 앵커=U | 53.3 | 65.5 | **81.8** | 80.1 | 72.2 | 69.3 |

앵커=N AUROC: 63.7 → **83.8** → 82.3 → 71.9 → 74.1

**2단계에서 주 지표 AUPRC 가 두 배가 됐고, 3·4단계는 그걸 넘지 못했다.**
계층별 지표를 주로 잡은 것도 옳았다 — 전체 AUPRC 는 +10.4 인데 주 계층은 +23.1 로,
전체만 봤으면 기여를 절반 이하로 과소평가했다.

**단계별 증분 (앵커=N AUPRC)**

| 비교 | Δ | 읽는 법 |
|---|---:|---|
| 2 − 1 | **+23.1** | 요약 피처가 룩업표를 크게 넘는다 |
| 3 − 2 | **−2.7** | **원시 시계열이 요약을 못 넘었다** (단, RASS+진정제 채널뿐) |
| 4 − 3 | **−13.9** | 개념 병목의 비용. 개념 5개(6개 중)라 상한값이다 |
| 4b − 4 | **−3.9** | 미관찰 앵커를 Assessable 로 살린 것이 **도움이 안 됐다** |

> ⚠️ 3단계가 2단계를 못 넘은 것은 표 형태 데이터에서 흔한 결과이지만, **여기서는 판정으로 쓸 수 없다** —
> mobility/pain/GCS 채널이 빠진 상태라 시계열 모델이 굶고 있다. 재추출 후 다시 봐야 한다.
...
6. **3·4단계가 2단계를 못 넘었다.** 지금 상태로는 시계열도 개념층도 XGBoost 를 못 이긴다. 채널이 반쪽(RASS+진정제)이고 개념도 5/6 이라 **아직 판정이 아니지만**, 재추출 후에도 같으면 논리층 이전에 설계를 다시 봐야 한다.
...
| 4 | XGBoost·BiLSTM 실행해 시계열의 추가 가치 확인 | ✅ 완료 — **재추출 후 재실행 필요** |
...
**순서 제안**: 1(재추출) 과 7(LTN 변환) 은 독립이라 **병렬**로 간다.
재추출이 끝나면 2·3·4단계를 그대로 다시 돌려(스크립트 있음) mobility/pain/GCS 의 증분을 재고,
7 이 끝나면 5단계(LTN)를 붙인다 → 8 게이트. **6단계(LNN)는 게이트 통과 후에만.**
