...
> **신경망은 backbone 을 뭘로 바꿔도 XGBoost 를 못 넘습니다** — BiLSTM 45.5, 요약 MLP 45.8,
> 같은 입력의 XGB 47.6. 시퀀스 인코더가 문제가 아니라 표 형태 데이터에서 GBDT 가 이기는 것입니다.
...
| 계층 | 기저% | 1 lookup | **2 XGB** | 3 BiLSTM | 3m MLP | 4 CBM(LSTM) | 4m CBM(MLP) |
|---|---:|---:|---:|---:|---:|---:|---:|
| 전체 | 31.0 | 71.6 | **82.8** | 81.6 | 81.5 | 70.4 | 60.3 |
| **앵커=N** (주 지표) | 12.0 | 23.0 | **48.9** | 45.4 | 45.8 | 30.4 | 27.2 |
| 앵커=P | 78.1 | 83.0 | **90.6** | 89.8 | 89.7 | 85.0 | 82.6 |
| 앵커=U | 53.3 | 65.5 | **82.3** | 81.0 | 80.6 | 71.0 | 55.7 |

앵커=N AUROC: 63.7 → **85.2** → 83.5 → 83.5 → 71.0 → 63.9
...
| 모델 | 평균 ± SD | 시드별 |
|---|---:|---|
| 3 BiLSTM | 45.5 ± 0.1 | 45.4 / 45.4 / 45.5 |
| 3m MLP | 45.8 ± 0.1 | 45.8 / 46.0 / 45.7 |
| 4 CBM(LSTM) | **28.1 ± 2.5** | 30.4 / 29.2 / 24.6 |
| 4m CBM(MLP) | **25.4 ± 1.2** | 27.2 / 24.3 / 24.8 |

> 🔴 **직전 제안(“backbone 을 요약 MLP 로 바꾸면 XGB 에 가까워진다”)은 틀렸다. 철회한다.**
> 개념층 없이 MLP − BiLSTM = **+0.3** (SD 0.1) 로, 실재하지만 무의미한 크기다.
> 같은 입력을 받은 XGB 는 47.6 이고 MLP 는 45.8 — **−1.8 의 격차는 시퀀스 인코더 탓이 아니다.**
> 표 형태 데이터에서 GBDT 가 신경망을 이기는 것이고, backbone 을 바꿔 메울 수 있는 종류가 아니다.
> CBM 에서는 −2.7 이지만 CBM 자체의 SD 가 2.5 라 **차이라고 말할 수 없다.**
> → **backbone 은 BiLSTM 을 유지해도 무방하다.** 5·6단계는 XGB 보다 ~2점 낮은 자리에서 출발한다고 보면 된다.

> ⚠️ **CBM 은 SD 가 2.5 다** (비CBM 은 0.1). 5차원 sigmoid 병목이 학습을 불안정하게 만든다.
> **4~6단계는 단일 시드로 비교하면 안 된다** — 평가 프로토콜에 반영했다(§3).
...
6. **신경망이 XGBoost 를 못 넘는 것은 backbone 문제가 아니다.** BiLSTM·MLP 둘 다 −2 안팎이고 backbone 교체 효과는 +0.3(SD 0.1)에 불과하다. **5·6단계는 XGB 보다 ~2점 낮은 자리에서 시작한다.** 개념 병목이 여기서 −18 을 더 깎는데, 이건 개념 5/6 이라 상한값이다.
...
**순서 제안**: (a) backbone 교체는 해봤고 효과가 없어 철회했다(§1). 남은 임계경로는 하나다 —
**컴파일된 공리 19개를 실제 학습 손실에 연결하는 것.**
그 전에 **개념 병목의 −18 을 줄일 수 있는지**를 먼저 보는 편이 낫다 — 개념 5/6 이라 Mobility·Pain 개념을 정의해 넣으면 병목이 넓어진다. 5단계는 이 병목 위에 얹히므로 여기가 좁으면 같이 눌린다.
