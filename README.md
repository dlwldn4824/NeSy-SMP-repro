...
> **서두(§1–5):** 교수님·노션·한글 보고용 요약 — *왜 달랐는지*와 *결과를 어디까지 해석할 수 있는지*를 먼저 제시한다.  
> **이하(상세 설명):** 기존 비교표·파이프라인·Table 결과·해석을 그대로 유지한다. 내용을 대체하거나 삭제하지 않았다.
...
- 교수님용 보고서: [`docs/NeSy-SMP_REPRO_REPORT.md`](docs/NeSy-SMP_REPRO_REPORT.md)
...
| 구분 | 논문 | 현재 재현 |
| --- | --- | --- |
| 환자 선정 | Sepsis-3 | 1차 ICD → 2차 Sepsis-3 근사 |
| 환자 수 | 약 19,328명 | ICD 약 9,959명 / S3 근사 약 18,899명 |
| 사망률 | 약 18% | ICD 26.2% / S3 근사 18.6% |

초기에는 ICD 진단코드를 이용하여 패혈증 환자를 선정하였다. 그러나 환자 수와 사망률이 논문과 크게 달랐다.

이에 두 번째 재현에서는 **감염 의심과 장기 기능 악화를 함께 고려하는 Sepsis-3 기준에 가깝게 환자군을 재구성**하였다.

그 결과 환자 수와 사망률은 논문과 상당히 유사해졌다.

> **논문:** 약 19,328명 / 사망률 약 18%  
> **재현:** 약 18,899명 / 사망률 18.6%

다만 논문의 최종 환자 목록 및 완성된 환자 선정 코드가 공개되지 않았기 때문에, 현재 환자군이 논문과 **완전히 동일한 환자 집단이라고 보장할 수는 없다.**

따라서 현재 데이터는 **Sepsis-3 근사 코호트**로 구분하였다.

---

### 2.2 동반질환 23개

논문에서는 환자의 임상노트를 자연어 처리하여 **23개 동반질환 정보를 0/1 형태로 추출**하여 모델 입력에 사용하였다.

현재 재현에서는 논문에서 사용한 임상노트 NLP 결과가 제공되지 않아 주요 실험에서 해당 23개 변수를 모두 0으로 설정하였다.

추가적으로 ICD 진단코드를 이용하여 동반질환을 추정한 **ICD proxy 데이터**도 구축하여 6시간 조건에서 비교하였다.

그러나 ICD proxy를 적용해도 NeSy-SMP 성능에는 큰 변화가 나타나지 않았다.

이는 어디까지나 ICD 기반 대체 변수의 결과이며, **논문의 임상노트 NLP 기반 동반질환 정보와 동일한 입력이라고 볼 수 없다.**

---

### 2.3 학습용 CSV 직접 생성

공개 GitHub 코드는 이미 전처리가 완료된 **lead time별 CSV가 존재한다고 가정**하고 모델을 학습한다.

하지만 해당 CSV가 공개되어 있지 않아 다음 과정을 직접 구현하였다.

> MIMIC-IV → 환자군 선정 → 임상 이벤트 추출 → 24시간 관찰 구간 생성 → Wide 형태 변환 → 동반질환 결합 → 모델 입력 CSV

사망 환자는 사망 시점에서 각각 6·12·24·48시간을 제외한 뒤 직전 24시간 데이터를 사용하였다.

생존 환자는 ICU 입원 기간 중 가능한 24시간 구간을 선택하였다.

이 과정에서 재실행할 때마다 다른 환자 구간이 선택되는 것을 방지하기 위해 **난수 seed를 32로 고정**하였다.

---

### 2.4 전처리 및 모델 입력 구조 보완

직접 생성한 CSV와 공개 코드가 기대하는 입력 형식 사이에 일부 차이가 있어 다음과 같은 보완을 수행하였다.

* 존재하지 않는 검사 및 동반질환 열을 0으로 생성
* 이벤트 범주 관련 입력 오류 방지
* 시계열 입력 최대 길이를 230으로 제한
* Risk MLP 입력 크기를 실제 생성된 입력 길이에 대응하도록 일반화

이러한 수정은 **공개 모델을 현재 구축한 데이터에서 실행하기 위한 호환성 처리**이다.

특히 누락된 열을 0으로 처리한 방식은 논문의 실제 결측값 처리와 동일하다고 확인된 것이 아니므로, 최종 재현에서는 별도의 결측률 및 입력 변수 검증이 필요하다.

---

### 2.5 Epoch 및 Checkpoint 차이

계산 자원과 실행 시간을 고려하여 초기 재현에서는 학습 횟수를 축소하였다.
...
Checkpoint 처리에도 차이가 있다.

공개 코드에서는 LTN과 NeSy-SMP가 validation 성능이 가장 좋은 모델을 저장하고 다시 불러와 test하는 반면, BiLSTM은 best 모델을 저장하지만 test 직전에 다시 불러오지 않는다.

현재 재현에서는 모델 간 평가 조건을 통일하기 위해 **BiLSTM을 포함한 모든 모델에서 validation macro-F1이 가장 높은 checkpoint를 불러와 test**하도록 하였다.

따라서 BiLSTM은 공개 GitHub 원본보다 성능이 좋은 학습 시점에서 평가되었을 가능성이 있다.

향후 최종 비교에서는 **원본 GitHub 방식과 checkpoint를 통일한 방식을 모두 실행하여 차이를 확인할 예정**이다.

---

### 2.6 실험 범위

논문에서는 6·12·24·48시간 lead time을 모두 평가하였다.

현재 재현에서는 먼저 6시간 조건을 중심으로 전체 파이프라인과 평가 지표가 정상적으로 동작하는지 검증하였다.

* **ICD 코호트:** 6시간 조건을 중심으로 Table 1 상세 비교
* **Sepsis-3 근사 코호트:** 6·12·24·48시간 전체 lead time 결과 확인

계산 비용을 줄이면서 데이터 구축부터 모델 평가까지의 전체 재현 가능성을 우선 검증하기 위한 단계였다.

---

## 3. 유지한 핵심 모델 구조

데이터 전처리와 일부 학습 조건에는 차이가 있지만, **NeSy-SMP의 핵심 학습 구조는 공개 GitHub 구현을 유지**하였다.

특히 데이터 기반 학습과 의료 지식 기반 학습의 비율은 공개 코드와 동일하게 적용하였다.

> **Data weight = 0.8**  
> **Knowledge weight = 0.2**

즉 현재 결과의 차이는 NeSy-SMP 자체를 새로운 모델로 변경해서 발생한 것이 아니라, 주로 다음 조건의 차이에서 발생할 가능성이 있다.

**환자군 구성 → 동반질환 정보 → 전처리 → Epoch/Checkpoint**

---

## 4. 현재 재현의 해석 범위

현재까지 공개 코드의 5개 모델을 실제 MIMIC-IV 데이터에서 end-to-end로 실행하고, Sepsis-3에 가까운 규모의 환자군을 재구축하여 6·12·24·48시간 예측 결과를 확인하였다.

특히 lead time이 증가할수록 성능이 감소하는 전체적인 경향은 논문과 동일하게 나타났다.

다만 현재 실험 조건에서는 **논문에서 보고된 NeSy-SMP의 상대적인 성능 우위가 명확하게 재현되지는 않았다.**

현재 결과만으로 NeSy-SMP의 효과가 없다고 판단하기보다는, 논문과 현재 재현 사이에 남아 있는 다음 조건을 분리하여 검증할 필요가 있다.

1. Sepsis-3 환자군의 정확한 재구성
2. 임상노트 NLP 기반 23개 동반질환 복원
3. 논문과 GitHub의 Weak Anchoring 구현 차이 검증
4. Epoch 및 Checkpoint 조건 통일
5. 결측값 및 입력 변수 처리 방식 검증
...
| 원 논문과 동일 Epoch 재실행 | 추가 재현 필요 |

### 현재 재현 단계

> **공개 GitHub의 모델 학습 루프와 손실 구조(`w_D=0.8`, `w_K=0.2`)는 유지하였으나, 공개되지 않은 코호트·동반질환·학습 CSV를 직접 재구축하고 일부 학습 조건을 조정하였다. 따라서 현재 결과는 논문의 완전한 수치 복제가 아닌 부분 재현이며, 논문과 공개 구현 사이의 차이를 식별하고 후속 검증 항목을 구체화한 단계이다.**

교수님께 보여드릴 때는 특히 **§2(논문과 다르게 재현한 주요 부분)**와 **§5(재현 진행 상태)**가 핵심이다.

---

# 상세 설명: 논문 · 공개 GitHub · 재현 비교

아래는 기존 README의 상세 비교·파이프라인·결과·해석이다. 서두(§1–5)를 보강한 뒤에도 **삭제하지 않고 유지**한다.
...
## D8. 논문과 GitHub의 Weak Anchoring 불일치
...
### D12.2 Sepsis-3 근사 코호트 — 전체 lead time
